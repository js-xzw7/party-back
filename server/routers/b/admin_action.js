...
module.exports = function(dbo){
    ...
    this.addAdminPost = async function(req) {
        ...
        try {
            ...
            //获取参数
            let params = req.body,
                {login_code,login_password,dep_id,avatar} = params;
            ...
            if(!avatar){
                ...
                avatar = `http://${ip}:${config.system.bind_port}${ENUM.FILE_PARAMS.FILE_PATH}${ENUM.FILE_PARAMS.USER_IMG}`
            }
            ...
            return Result.Ok('成功！',{login_code});
        } catch (error) {
    ...
    this.loginAdminPost = async function(req) {
        ...
        try {
            ...
            let admin_info = await TBUser.findOne({where:{
                login_code,
                login_password,
                role:'A'
            }});
            ...
            return Result.Ok('登录成功！',{login_code});
        } catch (error) {
            logger.error('失败！',error);
            return Result.Error('失败',error.message);
        }
